...
export abstract class Key<K extends ObjectsKeyType> {
    ...
    static async generateBits(password: string, salt: string): Promise<Uint8Array> {
        ...
        let result = await argon2.hash({ pass: utf8.encode(password), salt: utf8.encode("saltysalt" + salt), hashLen: 32, mem: 131072, time: 1, parallelism: 1, type: argon2.ArgonType.Argon2id });
        return result.hash;
    }
}